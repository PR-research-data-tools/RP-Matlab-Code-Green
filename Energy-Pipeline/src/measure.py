...
# Location variables
PATH_TO_ENERGIBRIDGE_BINARY = "path_to_provide"
PATH_TO_PROJECT_FOLDER = "path_to_provide"
NAME_DOCKER_IMAGE = "matlab-r2021b-toolbox"
...
def execute_matlab_script_and_measure_energy(execution, count):
    ...
    script_command = [PATH_TO_ENERGIBRIDGE_BINARY ,"--summary" ,"--output", 
    PATH_TO_PROJECT_FOLDER + "/output/energy_metrics_" + str(folder_script) + "-" + str(script_name) + "_" + str(count) + ".csv" ,
    "-c" ,PATH_TO_PROJECT_FOLDER + "/output/output_simulation_" + str(folder_script) + "-" + str(script_name) + "_" + str(count) + ".txt" ,
    "docker" ,"run", "--rm", "-v", PATH_TO_PROJECT_FOLDER + "/sampling:/sampling" , 
    "-v", PATH_TO_PROJECT_FOLDER + "/output:/output", 
    "-v" ,PATH_TO_PROJECT_FOLDER + "/matlab.dat:/licenses/license.lic", 
